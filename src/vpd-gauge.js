...
import {
    ...
    html,
    css,
  } from 'lit'; // Use direct import now
  ...
  class VpdGaugeCard extends LitElement {
    ...
    _calculateSegments = () => {
      ...
      if (!hass || !config || !config[CONF_MIN_ENTITY] || !config[CONF_MAX_ENTITY]) {
          console.warn("VPD Gauge: Hass or required config missing for segment calculation.");
      ...
      const staticLowThreshold = config.static_low_threshold ?? DEFAULT_STATIC_LOW_THRESHOLD;
      const staticHighThreshold = config.static_high_threshold ?? DEFAULT_STATIC_HIGH_THRESHOLD;
  
      const colorExtremeLow = config.color_extreme_low || DEFAULT_COLOR_EXTREME_LOW;
      ...
      const colorExtremeHigh = config.color_extreme_high || DEFAULT_COLOR_EXTREME_HIGH;
      ...
      if (minThresholdState && !isNaN(parseFloat(minThresholdState.state))) {
          minThreshold = parseFloat(minThresholdState.state);
           ...
           console.warn(`VPD Gauge: Invalid state for min_entity (${minEntityId}), using static fallback ${minThreshold}`);
      }
      ...
      if (maxThresholdState && !isNaN(parseFloat(maxThresholdState.state))) {
          maxThreshold = parseFloat(maxThresholdState.state);
           ...
           console.warn(`VPD Gauge: Invalid state for max_entity (${maxEntityId}), using static fallback ${maxThreshold}`);
      }
      ...
      if (finalSegments.length === 0 || finalSegments[0].from > gaugeMin) {
          ...
          if (finalSegments.length > 1 && finalSegments[1].from === finalSegments[0].from) {
              finalSegments.shift();
          }
      }
  
      console.log(`VPD Gauge (${config.entity}): MinT=${minThreshold}, MaxT=${maxThreshold}, Segments=`, finalSegments);
    ...
    render() {
      ...
      console.log("Render method called. Hass state available:", !!this.hass, "Config available:", !!this.config);
      ...
      if (!this.hass || !this.config || !this.config[CONF_ENTITY]) {
        ...
        // Optionally render a config warning
         return html`<ha-card header="VPD Gauge"><div class="warning">Please configure the required entities.</div></ha-card>`;
      }
      ...
      console.log(`Rendering VPD Gauge: Entity=${this.config[CONF_ENTITY]}, StateObj=`, stateObj, `Value=${value}`);
  
  
      if (stateObj === undefined) {
          return html`
              <ha-card header="${this.config[CONF_NAME] || 'VPD Gauge'}">
                  <div class="warning">Entity not found: ${this.config[CONF_ENTITY]}</div>
              </ha-card>
          `;
      }
      ...
      if (value === undefined || isNaN(value)) {
           ...
           return html`
              <ha-card header="${this.config[CONF_NAME] || 'VPD Gauge'}">
                  <div class="warning">Invalid state for ${this.config[CONF_ENTITY]}: ${stateObj.state}</div>
              </ha-card>
          `;
      }
    ...
    static getConfigElement() {
      ...
      import("./vpd-gauge-card-editor.js").then(() => {
    ...
    static getStubConfig(hass, entities, entitiesFallback) {
      ...
      const sensorEntity = entities.find(e => e.startsWith("sensor.") && e.includes("vpd")) || entitiesFallback.find(e => e.startsWith("sensor."));
      const minEntity = entities.find(e => e.startsWith("number.") && e.includes("min")) || entitiesFallback.find(e => e.startsWith("number."));
      const maxEntity = entities.find(e => e.startsWith("number.") && e.includes("max")) || entitiesFallback.find(e => e.startsWith("number."));
      return {
  ...
  console.log("VPD Gauge Card Loaded Successfully");