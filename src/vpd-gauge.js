...
// --- Keep this import to trigger loading ---
...
class VpdGaugeCard extends LitElement {
  ...
  setConfig(config) {
    ...
    const defaults = { gauge_min: DEFAULT_GAUGE_MIN, gauge_max: DEFAULT_GAUGE_MAX, static_low_threshold: DEFAULT_STATIC_LOW_THRESHOLD, static_high_threshold: DEFAULT_STATIC_HIGH_THRESHOLD, color_extreme_low: DEFAULT_COLOR_EXTREME_LOW, color_low: DEFAULT_COLOR_LOW, color_good: DEFAULT_COLOR_GOOD, color_high: DEFAULT_COLOR_HIGH, color_extreme_high: DEFAULT_COLOR_EXTREME_HIGH, name: "", };
    this.config = { ...defaults, ...config, };
    delete this.config[CONF_NEEDLE]; // Remove old config key if present
  ...
  render() {
    // console.log("Render method called. Hass:", !!this.hass, "Config:", !!this.config); // Optional: Keep for debug
    ...
    // console.log(`Rendering: Entity=${this.config[CONF_ENTITY]}, StateObj=`, stateObj, `Value=${value}`); // Optional: Keep for debug
  ...
  static get styles() { /* Keep styles as before */ return css`ha-card{height:100%;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden}.card-content{display:flex;justify-content:center;align-items:center;padding:16px;flex-grow:1}ha-gauge{width:100%;max-width:250px;--gauge-color:var(--primary-text-color)}.warning{padding:16px;text-align:center;color:var(--error-color)}`; }
}
