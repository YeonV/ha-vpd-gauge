// src/vpd-gauge.js - FINAL VERSION BASED ON WORKING BASE + FIXES
...
// const CONF_NEEDLE = "needle"; // REMOVED NEEDLE
...
class VpdGaugeCardEditor extends LitElement {
    ...
    firstUpdated(changedProperties) {
        ...
        if (!this._initialized) { // Ensure setup runs only once
            this._storeElementReferences();
            this._attachInputListeners();
            this.loadEditorValues();
            this._initialized = true;
             console.log("[Editor] Initialized and values loaded.");
        }
    }
    ...
    _storeElementReferences() {
        ...
        // this._elements.inputs.needle = root.querySelector('#needle'); // REMOVED NEEDLE
    ...
    _attachInputListeners() {
        ...
        // this._elements.inputs.needle?.addEventListener('change', this._valueChanged); // REMOVED NEEDLE
    ...
    loadEditorValues() {
        ...
        // const setChecked = (el, value) => { if (el) el.checked = value; }; // REMOVED NEEDLE
        ...
        // setChecked(this._elements.inputs.needle, this._config[CONF_NEEDLE] !== false); // REMOVED NEEDLE
        ...
        for (const group in this._elements.colors) {
            ...
            if (picker) {
                ...
                // Use window[] to access global const by string name
        ...
        // Ensure hass is passed to pickers
        if (this._hass && this._elements.pickers) {
             Object.values(this._elements.pickers).forEach(picker => {
                if (picker) picker.hass = this._hass;
            });
        }
    ...
    _valueChanged = (ev) => {
        ...
        if (value === undefined || value === "" || (typeof value === 'number' && isNaN(value))) {
            ...
            if (configKey !== CONF_ENTITY && configKey !== CONF_MIN_ENTITY && configKey !== CONF_MAX_ENTITY) {
                 ...
                 newConfig[configKey] = ""; // Keep required keys empty
            }
        } else {
    ...
    _colorChanged = (ev) => {
        ...
        if (value) {
            newConfig[configKey] = value.toUpperCase(); // Store as uppercase hex
        } else {
            delete newConfig[configKey];
        }
    ...
    render() {
      ...
      // Define values directly in render scope for clarity
      ...
      // const needle = this._config[CONF_NEEDLE] !== false; // REMOVED NEEDLE
...
if (!customElements.get('vpd-gauge-card-editor')) {
    ...
    console.info('%c VPD-GAUGE-CARD-EDITOR %c Defined', /* ... */);
}
...
class VpdGaugeCard extends LitElement {
  ...
  // ------------------------------------------------------
    ...
    static getConfigElement() {
      console.log("getConfigElement: Creating vpd-gauge-card-editor element.");
